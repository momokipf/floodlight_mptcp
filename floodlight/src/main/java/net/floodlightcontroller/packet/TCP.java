...
public class TCP extends BasePacket {
    ...
    public static final int MP_CAPABLE = 0;
    public static final int MP_JOIN = 1;
    public static final int DSS = 2;
    public static final int MP_ADD_ADDR =3;
    public static final int MP_REM_ADDR = 4;
    public static final int MP_PRIO=5;
    public static final int MP_FAIL = 6;
    public static final int MP_FASTCLOSE =7;
    ...
    public boolean isMPTCPEnable(){
        if(options.length<=20){
            return false;
        }
        //byte kindOptions = options[20];
        return options[20] == (byte)0x1e;
    }


    public int getMptcpSubtype(){

        int subtype =  (options[22]>>4)&(0x0F);
        log.info("subtype:{}",Integer.toUnsignedString(subtype));
        // type 0: MP_CAPABLE return 0 
        if(subtype == MP_CAPABLE){
        	log.info("MPTCP packages: subtype MP_CAPABLE");
            return MP_CAPABLE;
        }
        // type 1: MP_JOIN return 1
        else if(subtype == MP_JOIN){
        	log.info("MPTCP packages: subtype MP_JOIN");
            return MP_JOIN;
        }
        else if(subtype == DSS){
        	log.info("MPTCP packages: subtype DSS");
            return DSS;
        }
        else if(subtype == MP_ADD_ADDR ){
        	log.info("MPTCP packages: subtype MP_ADD_ADDR");
        }
        else if(subtype == MP_FAIL){
        	log.info("MPTCP packages: subtype MP_FAIL");
        }
        else if(subtype ==MP_FASTCLOSE){
        	log.info("MPTCP packages: subtype MP_FAIL");
        }
